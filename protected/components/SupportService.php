...
class SupportService {
    ...
    static function countQuestionsByUserId($user_id) {
        $sql = "SELECT COUNT(q.id) total FROM tbl_question q JOIN tbl_ques_support qs ON q.id=qs.ques_id AND qs.user_id=:user_id AND q.isdel='N'";
    ...
    static function listQuestionsByUserId($user_id, $offset=0, $rows=20) {
        $sql = "SELECT q.id,qp.img,q.createtime ques_time,u.id user_id,u.nick user_nick,u.head user_head,
                ...
                FROM (
                    ...
                    FROM tbl_question q JOIN tbl_ques_support qs ON q.id=qs.ques_id AND qs.user_id=:user_id AND q.isdel='N'
                    ORDER BY qs.id DESC limit $offset,$rows
                ) q
                JOIN (
                    -- 查询每个提问的最热图片
    ...
    static function countAnswersByUserId($user_id) {
        $sql = "SELECT COUNT(a.id) total FROM tbl_answer a JOIN tbl_ans_support af ON a.isdel='N' AND a.id=af.ans_id AND af.user_id=:user_id";
        $rows = DbUtils::query($sql,array(':user_id'=>$user_id));
        return empty($rows) ? 0 : intval($rows[0]['total']);
    }
    static function listAnswersByUserId($user_id, $offset=0, $rows=20) {
        $sql = "SELECT a.id,a.createtime ans_time,a.happens,a.occurdate,a.place,a.content,
                ...
                FROM (
                    ...
                    FROM tbl_answer a JOIN tbl_ans_support af ON af.user_id=:user_id AND af.ans_id=a.id AND a.isdel='N'
    ...
    static function countComparesByUserId($user_id) {
        $sql = "SELECT COUNT(c.id) total FROM tbl_compare c JOIN tbl_compare_support cs ON c.id=cs.compare_id AND cs.user_id=:user_id AND c.isdel='N'";
        $rows = DbUtils::query($sql,array(':user_id'=>$user_id));
        return empty($rows) ? 0 : intval($rows[0]['total']);
    }
    static function listComparesByUserId($user_id, $offset=0, $rows=20) {
        $sql = "SELECT c.id,c.title,c.createtime,u.id uid,u.nick,u.head,cd.id compare_detail_id,cd.img,
                ...
                FROM (
                    ...
                    FROM tbl_compare c JOIN tbl_compare_support cs ON cs.user_id=:user_id AND c.id=cs.compare_id AND c.isdel='N'
                    ORDER BY cs.id DESC LIMIT $offset,$rows
                ) c
    ...
    static function countTopicsByUserId($user_id) {
        $sql = "SELECT COUNT(t.id) total FROM tbl_topic t JOIN tbl_topic_support ts ON t.id=ts.topic_id AND ts.user_id=:user_id AND t.isdel='N'";
        $rows = DbUtils::query($sql,array(':user_id'=>$user_id));
        return empty($rows) ? 0 : intval($rows[0]['total']);
    }
    static function listTopicsByUserId($user_id, $offset=0, $rows=20) {
        $sql = "SELECT tp.id,tp.title,tp.cover,
                ...
                tc.celebrity_id,tt.total total_cele,c.head FROM (
                    SELECT tp.id,tp.title,tp.cover FROM tbl_topic tp JOIN tbl_topic_support ts ON tp.id=ts.topic_id AND ts.user_id=:user_id AND tp.isdel='N'
                    ORDER BY ts.id DESC LIMIT $offset,$rows
                ) tp
                LEFT JOIN (
                    -- 话题相关的最新6个名人
    ...
    /** 单图 */
    static function countPicturesByUserId($user_id) {
        $sql = "SELECT COUNT(id) total FROM tbl_img_support t WHERE user_id=:user_id";
        $rows = DbUtils::query($sql,array(':user_id'=>$user_id));
        return empty($rows) ? 0 : intval($rows[0]['total']);
    }
    static function listPicturesByUserId($user_id, $offset=0, $rows=20) {
        $sql = "SELECT id,url FROM tbl_img_support WHERE user_id=:user_id ORDER BY id DESC LIMIT $offset,$rows";
        return DbUtils::query($sql, array(':user_id'=>$user_id));
    }
}